...
- **记录历史** —— 每次你 "commit"（提交），Git 都会给当前所有文件拍一张快照。你可以随时回到任意一次提交。
- **分支管理** —— 你可以从主干分出一条"平行时空"来实验新功能，互不干扰，改好了再合并回去。
- **本地优先** —— 绝大多数操作都在本地硬盘上完成，速度极快。网络只在你需要推送或拉取代码时才用到。
- **完整性** —— 每次提交都会生成一个唯一的 SHA-1 哈希值，确保内容不会被意外篡改或丢失。

简单来说，**Git 是你电脑上的时光机**。

> [!tip]
>
> Git 原本是一个命令行软件，但本文借助 VS Code 的源代码管理功能，只需要点点按钮就能完成 Git 命令操作。
>
> 如果你用命令行操作，步骤也是一样的。
...
如果你使用了代理工具🪜来访问 Github，请在代理工具的设置页面找找它的端口是什么，记住这个端口号。
...
**从 Github 复制一个已有仓库到本地。**

1. 打开 Github 上的目标仓库页面
...
顾名思义，就是完成这个分支的开发任务，`feature/`分支开发新功能、`fix/`分支修 bug……
...
> **提交（Commit）是什么？**
>
> “提交”就像是给当前版本的代码设置一个“存档点”，便于后面回溯到这个版本。
>
> “提交”可以推送到远程仓库，便于你的队友也能切换到你的分支来查看情况。
>
> 正如你玩游戏会进行多次存档，做一个大的模块功能时也应该有多次提交。
>
> 比如写完当前模块的一部分、调试跑通了等等情况，都可以进行提交。

要提交，先暂存。暂存就像是打包——你可以**挑选**哪些文件进入这次提交，没暂存的文件不会跟着提交。

在源代码管理页面，Git 会识别出文件有哪些变化。点击文件名右侧的加号 `+`，表示暂存这个文件的改动。

![](images/commit-add.png)

> [!note]
>
> ❓**文件名旁边的字母是什么意思？详见 Q&A 章节 [文件名旁边的字母是什么意思？](#文件名旁边的字母是什么意思)。**

暂存后，在【消息】一栏写下这次提交做了什么，比如新增了什么功能，修复了什么问题等等。然后点击【提交】。
...
> **Pull Request 是什么？**
>
> 简单说：你在自己分支上做好了，向团队**喊一嗓子 "来审查我的代码"**，这就是 Pull Request（简称 PR）。它不仅是一个合并操作，更是一个**代码审查**的环节——团队可以在 PR 页面里逐行评论、讨论、要求修改。
>
> PR 的生命周期一般是：
>
> 1. **创建 PR** —— 选择你的开发分支和目标分支（目标分支通常是 `main`）
> 2. **代码审查** —— 团队成员逐行审阅，提出意见
> 3. **修改完善** —— 根据反馈修改代码
> 4. **合并** —— 审查通过后，把代码并入主分支
>
> **PR 不是为了"找茬"，而是为了"把关"**——发现潜在问题、让团队了解项目变动，让大家都熟悉你写的代码。

接下来的操作，需要打开 Github 来完成了。
...
审查不是在 VS Code 里做的，而是在 **Github 的 PR 页面**上完成的。操作路径如下：

打开 PR 页面，在仓库的 Pull Request 列表中找到这个 PR 并点击进入。

切换到 **"Files Changed"** 标签，这里会以对比视图（Diff 试图）展示这个分支改动了哪些文件和代码行（绿色 = 新增，红色 = 删除）。

审查者如果发现某一行有问题，把鼠标悬停在那一行上，点击左侧的 `+` 号，可以针对这一行发表评论。

提交审查结果，看完所有文件后，点击右上角的 **"Submit review"** 按钮，弹出三种选择：

![](images/pr-review.png)

   | 选项 | 含义 | 什么时候用 |
   |---|---|---|
   | **Comment（评论）** | 只发评论，不阻止合并 | 有疑问但不需要修改时 |
   | **Approve（批准）** | 代码没问题，同意合并 | 审查通过后 |
   | **Request Changes（请求修改）** | 发现问题，必须修改后才能合并 | 代码有 bug 或设计问题时 |
...
>
> 前者在主分支上产生复杂的合并节点，而 Squash 会把你的开发分支上的提交“压缩”到一起，再放到主分支上，这样主分支节点图会更干净、清晰。
...
在源代码管理页面做一次拉取操作，把主分支同步到最新状态。同时也通知其他队友切到主分支并拉取，以便同步主分支最新的修改。

<img src="images/pull.png" style="zoom: 50%;" />

至此，一次开发流程就结束了。后续写其他部分的代码，就回到第1步开始。
